...
export default function IconEdit() {
  ...
  const fetchIcon = async () => {
    ...
    try {
      ...
      console.error(err);
      alertError("Gagal memuat icon", "Periksa koneksi atau coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(res.data);
      await alertSuccess("Berhasil", res.data?.message || "Icon diperbarui.");
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white via-blue-50 to-blue-100">
        <p className="text-sm text-gray-600">Memuat data icon...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50 to-blue-100 text-gray-900 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 text-center md:text-left">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Edit Icon</h1>
          <p className="text-gray-600 mt-2">Perbarui data icon dan simpan perubahan.</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                Nama Icon <span className="text-blue-600">*</span>
              </label>
              <input
                type="text"
                name="nama_icon"
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">
                File Icon <span className="text-blue-600">*</span>
              </label>
              <input
                type="file"
                accept="image/*"
              ...
              />
              {form.file_path_icon && (
                <p className="text-xs text-gray-500">Saat ini: {form.file_path_icon}</p>
              )}
            </div>
