...
import { fetchHotels as fetchAdminHotels } from "../../../api/admin/apiAdminHotels";
import { createKamar } from "../../../api/admin/apiAdminKamars";
...
export default function KamarCreate() {
  ...
  const validateForm = () => {
    ...
    if (!form.harga || Number.isNaN(harga) || harga <= 0) return "Harga harus lebih dari 0.";
    const kapasitas = Number(form.kapasitas);
    if (!kapasitas || Number.isNaN(kapasitas) || kapasitas < 1) return "Kapasitas minimal 1.";
  ...
  const handleSubmit = async (e) => {
    ...
    const confirm = await alertConfirm({
      title: "Simpan data?",
      text: "Data kamar akan ditambahkan.",
    ...
    try {
      ...
      const res = await createKamar(payload);
      console.log(res);
      await alertSuccess("Berhasil", res?.message || "Kamar ditambahkan.");
      setForm({
        id_hotel: "",
        nama_kamar: "",
        nomor_kamar: "",
  ...
  return (
    <AdminFormShell
      title="Tambah Kamar"
      subtitle="Lengkapi data kamar di bawah ini"
