...
export default function GambarHotelList() {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error(err);
      alertError("Gagal memuat gambar hotel", "Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await useAxios.delete(`/gambar-hotel/delete/${id}`);
    ...
    <div className="min-h-screen flex bg-[#f4f6fb] text-slate-800">
      ...
      <main className="flex-1 p-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Manajemen Gambar Hotel</h2>
            <p className="text-slate-500 text-sm">Kelola foto-foto hotel yang tampil di aplikasi.</p>
          </div>
          <button
            onClick={() => navigate("/admin/gambar-hotel/create")}
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  items.map((item, idx) => (
                    <tr key={item.id_gambar_hotel || idx} className="hover:bg-slate-50">
                      <td className="px-4 py-3">{item.id_gambar_hotel || idx + 1}</td>
                      <td className="px-4 py-3 font-semibold text-slate-900">
                        {item.hotel?.nama_hotel || item.nama_hotel || "-"}
                      </td>
                      <td className="px-4 py-3 text-slate-800">{item.nama_gambar_hotel}</td>
                      <td className="px-4 py-3 text-slate-700">
                        {item.keterangan_gambar_hotel || "-"}
                      </td>
                      <td className="px-4 py-3 text-slate-700 break-all">{item.file_path_gambar_hotel}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-3">
                          <button
                            type="button"
                            className="text-blue-600 hover:text-blue-700"
                            onClick={() => navigate(`/admin/gambar-hotel/edit/${item.id_gambar_hotel || ""}`)}
                          >
