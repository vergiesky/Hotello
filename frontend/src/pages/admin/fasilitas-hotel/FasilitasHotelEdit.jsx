  ...
  const fetchFacility = async () => {
    ...
    try {
      ...
      const filled = {
        ...
        keterangan_fasilitas_hotel: data.keterangan_fasilitas_hotel || data.deskripsi || "",
      };
      setForm(filled);
      setInitialForm(filled);
    } catch (err) {
      console.error(err);
      alertError("Gagal memuat fasilitas", "Periksa koneksi atau coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-white via-blue-50 to-blue-100">
        <p className="text-sm text-gray-600">Memuat data fasilitas...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50 to-blue-100 text-gray-900 py-10 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8 text-center md:text-left">
          ...
          <h1 className="text-3xl font-bold text-gray-900">Edit Fasilitas Hotel</h1>
          <p className="text-gray-600 mt-2">Perbarui data fasilitas dan simpan perubahan.</p>
        </div>
