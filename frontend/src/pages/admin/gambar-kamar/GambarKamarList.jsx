...
import { fetchGambarKamar, deleteGambarKamar } from "../../../api/admin/apiAdminGambarKamar";
...
export default function GambarKamarList() {
  ...
  const fetchData = async () => {
    try {
      const res = await fetchGambarKamar();
      setItems(res?.data?.data || res?.data || res || []);
    } catch (err) {
      console.error(err);
      toastError("Gagal memuat gambar kamar. Silakan coba lagi.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-[#f4f6fb] text-slate-800">
      ...
      <main className="flex-1 w-full p-6 md:p-8">
        <AdminListHeader
          title="Manajemen Gambar Kamar"
          subtitle="Kelola gambar setiap kamar"
          query={query}
              ...
              <tbody className="divide-y divide-slate-100">
                {loading ? (
                  ...
                  filtered.slice(start, start + pageSize).map((item, idx) => (
                    <tr
                      key={item.id_gambar_kamar || idx}
                      className="hover:bg-slate-50"
                    >
                      <td className="px-4 py-3">
                        {item.id_gambar_kamar || start + idx + 1}
                      </td>
                      <td className="px-4 py-3 font-semibold text-slate-900">
                        {item.kamar?.nama_kamar || item.nama_kamar || "-"}
                      </td>
                      <td className="px-4 py-3">
                        {item.file_path_gambar_kamar ? (
                          <img
                            src={getImageUrl(item.file_path_gambar_kamar)}
                            alt={
                              item.nama_gambar_kamar || "Preview gambar kamar"
                            }
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-center gap-3">
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/admin/gambar-kamar/edit/${
                                  item.id_gambar_kamar || ""
                                }`
                              )
                            }
                          >
